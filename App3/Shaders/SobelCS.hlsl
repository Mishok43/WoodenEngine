...
[numthreads(16, 16, 1)]
void SobelCS(int3 groupThreadID: SV_GroupThreadID,
				int3 dispatchThreadID : SV_DispatchThreadID)
{
	...
	for (int i = 0; i < 3; ++i)
	{
		for (int j = 0; j < 3; ++j)
		{
			int2 xy = dispatchThreadID.xy + int2(-1 + i, -1 + j);
			c[i][j] = gInput[xy];
		}
	}
	...
	// Gradient is (Gx, Gy). For each colorchannel, compute magnitude to  // get maximum rate of change.  
